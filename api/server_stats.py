...
async def get_single_server_stats(
    server_id: int,
    user_info: Dict = Depends(require_roles(["admin", "onboarding", "qa"])),
    client_id: Optional[int] = Query(None, description="Filter by specific client"),
    active_only: bool = Query(False, description="Show only active campaigns")
    ...
    """
    ADMIN: GET STATISTICS FOR A SPECIFIC SERVER
    
    Shows detailed statistics for a single server including:
    - All campaigns running on this server
    - Bot counts and active bot counts
    - Client information for each campaign
    - Extension numbers being used
    
    Filters:
    - client_id: Show only campaigns for a specific client on this server
    - active_only: Show only active campaigns on this server
    """
    pool = await get_db()
    
    async with pool.acquire() as conn:
        # First check if server exists
        server_check = await conn.fetchrow(
            "SELECT id, ip, alias, domain FROM servers WHERE id = $1",
            server_id
        )
        ...
        # Build filters
        where_clauses = [f"s.id = ${2}"]
        params = [one_minute_ago, server_id]
        param_count = 2
        ...
        # Query for this specific server
        query = f"""
            WITH campaign_activity AS (
                SELECT 
                    client_campaign_model_id,
        ...
        for row in rows:
            ...
            total_bots += server_bot_count
            if is_active:
                active_bots += server_bot_count
            
            campaigns_list.append(CampaignOnServer(
                campaign_id=row['campaign_id'],
                campaign_name=row['campaign_name'],
                model_name=row['model_name'],
                client_id=row['client_id'],
        ...
        return ServerStats(
            ...
            server_ip=server_check['ip'],
            server_alias=server_check['alias'],
            server_domain=server_check['domain'],
            total_campaigns=len(campaigns_list),
            active_campaigns=sum(1 for c in campaigns_list if c.is_active),
